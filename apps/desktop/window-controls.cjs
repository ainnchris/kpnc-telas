...
function setupWindowControls({ipcMain,getMain,clipboard}){
  ...
  ipcMain.handle('meet:copy-invite',async(event,code)=>{windowFor(event);const url=inviteURL(code);if(!url)throw new Error('Convite inválido.');await clipboard.writeText(url);return true;});
