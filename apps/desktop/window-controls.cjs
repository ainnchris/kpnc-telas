...
function inviteURL(code,site=SITE){return typeof code==='string'&&/^[a-z0-9-]{6,64}$/.test(code)?`${site}/?room=${encodeURIComponent(code)}`:null;}
...
function setupWindowControls({ipcMain,getMain,clipboard,site=SITE}){
  function windowFor(event){const win=getMain();if(!win||win.isDestroyed()||event.sender!==win.webContents||event.senderFrame!==win.webContents.mainFrame||!isMeetURL(win.webContents.getURL(),site))throw new Error('Forbidden');return win;}
  ipcMain.handle('meet:copy-invite',async(event,code)=>{windowFor(event);const url=inviteURL(code,site);if(!url)throw new Error('Convite inválido.');await clipboard.writeText(url);return true;});
