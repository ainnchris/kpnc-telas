...
try {
  ...
  const statusPath=`/api/join-requests/${join.requestId}?room=${encodeURIComponent(auth.room)}`;
  const waiting=await request(statusPath,{hostKey:join.requestSecret});assert.equal(waiting.data.status,'waiting');assert.equal(waiting.data.token,undefined);
  ...
  const approved=await request(statusPath,{hostKey:join.requestSecret});assert.equal(approved.data.status,'approved');assert.equal(approved.data.host,false);
  ...
  const deniedStatus=await request(`/api/join-requests/${denied.data.requestId}?room=${encodeURIComponent(auth.room)}`,{hostKey:denied.data.requestSecret});
