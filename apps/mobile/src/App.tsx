...
type RoleStatus={role:AdminRole;permissions:MemberPermissions;roles:Record<string,AdminRole>;memberPermissions:Record<string,MemberPermissions>};
...
function Meeting({auth,onLeave,theme,setTheme}:{auth:Auth;onLeave:()=>void;theme:ThemeName;setTheme:(theme:ThemeName)=>void}) {
  ...
  async function decide(id:string,decision:'admit'|'deny') {await api(`/api/rooms/${auth.room}/requests/${id}/${decision}`,post({},adminKey));setPending(items=>items.filter(item=>item.id!==id))}
  async function toggleLock(){const result=await api<{locked:boolean}>(`/api/rooms/${auth.room}/settings`,post({locked:!locked},adminKey));setLocked(result.locked)}
  ...
  async function changeRole(p:Participant,role:'participant'|'cohost'){await api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/role`,post({role},adminKey))}
  async function transferHost(p:Participant){await api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/transfer`,post({},adminKey))}
  async function updatePermissions(p:Participant,key:keyof MemberPermissions){const current=memberPermissions[p.identity]||DEFAULT_MEMBER_PERMISSIONS;const next={...current,[key]:!current[key]};await api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/permissions`,post({permissions:next},adminKey));setMemberPermissions(values=>({...values,[p.identity]:next}))}
