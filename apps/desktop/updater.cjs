...
function setupUpdater({app,ipcMain,getMain}){
 ...
 ipcMain.handle('update:download',async event=>{
   ...
   downloading=(async()=>{
     ...
     let bytes=0,lastProgress=0;const total=Number(response.headers.get('content-length'))||0;
     const hash=crypto.createHash('sha256'),limit=new Transform({transform(chunk,_encoding,callback){bytes+=chunk.length;if(bytes>350*1024*1024)return callback(new Error('Arquivo excede o limite'));hash.update(chunk);if(Date.now()-lastProgress>250){lastProgress=Date.now();const main=getMain();if(main&&!main.isDestroyed()&&isMeetURL(main.webContents.getURL()))main.webContents.send('update:progress',{received:bytes,total,percent:total?Math.min(99,Math.floor(bytes/total*100)):null});}callback(null,chunk);}});
 ...
 ipcMain.handle('update:install',async event=>{
   ...
   try{await launchSilentInstaller(ready.file);}catch(error){await getMain().webContents.executeJavaScript('document.body.inert=false');throw error;}app.quit();return true;
 });
}
module.exports={setupUpdater,validUpdate,launchSilentInstaller};
