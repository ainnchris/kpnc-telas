...
type Message = {id:string;name:string;text:string;identity?:string;createdAt?:number;replyTo?:{id:string;name:string;text:string};system?:boolean};
...
type AdminRole='participant'|'cohost'|'host';
type MemberPermissions={microphone:boolean;camera:boolean;chat:boolean;screen:boolean};
type RoleStatus={role:AdminRole;permissions:MemberPermissions;roles:Record<string,AdminRole>;memberPermissions:Record<string,MemberPermissions>;locked:boolean};
type ModerationEvent={id:string;action:string;actorIdentity:string;actorRole:'host'|'cohost';targetIdentity?:string;targetName?:string;changes?:Partial<MemberPermissions>;createdAt:number};
const DEFAULT_MEMBER_PERMISSIONS:MemberPermissions={microphone:true,camera:true,chat:true,screen:true};
...
type EmojiCategory=keyof typeof MOBILE_EMOJIS;
const EMOJI_LABELS:Record<EmojiCategory,string>={recentes:'🕘 Recentes',rostos:'😀 Rostos',gestos:'👋 Gestos',coracoes:'❤️ Corações',animais:'🐶 Animais',comida:'🍕 Comida',atividades:'⚽ Atividades',objetos:'💡 Objetos',simbolos:'✨ Símbolos'};
const dark={bg:'#08111e',surface:'#142136f2',text:'#f2f6ff',muted:'#aab8ce',border:'#31425c',accent:'#4b91ff'};
const light={bg:'#f2f6fc',surface:'#fffffff2',text:'#13223b',muted:'#5c6d84',border:'#d5dfed',accent:'#2472e8'};
const palettes={light,dark,gray:{...dark,bg:'#292e36',surface:'#3a404af2',border:'#5e6672',muted:'#c5cbd4'},black:{...dark,bg:'#000000',surface:'#101010f5',border:'#303030',muted:'#b8b8b8'}};
...
function MessageText({text,color}:{text:string;color:string}){
  ...
  return <Text selectable style={{color,lineHeight:20}}>{parts.map((part,index)=>{
    if(!/^https?:\/\//i.test(part))return <Text key={index}>{part}</Text>;
    let safe='';try{const url=new URL(part);if(['http:','https:'].includes(url.protocol)&&!url.username&&!url.password)safe=url.href}catch{}
    return safe?<Text key={index} accessibilityRole="link" style={{color:'#438dff',textDecorationLine:'underline'}} onPress={()=>void Linking.openURL(safe)}>{part}</Text>:<Text key={index}>{part}</Text>;
  })}</Text>;
}
...
function errorMessage(error:unknown){
  ...
  if(/^(Informe|Código|Esta reunião|O anfitrião|O compartilhamento|O chat|A atualização|O modo|Não foi possível)/i.test(value))return value;
...
const E2EE_KEY_PATTERN=/^[A-Za-z0-9_-]{32}$/;
function base64Url(bytes:Uint8Array){const alphabet='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';let output='';for(let index=0;index<bytes.length;index+=3){const a=bytes[index],b=bytes[index+1],d=bytes[index+2],value=(a<<16)|((b||0)<<8)|(d||0);output+=alphabet[(value>>>18)&63]+alphabet[(value>>>12)&63]+(index+1<bytes.length?alphabet[(value>>>6)&63]:'')+(index+2<bytes.length?alphabet[value&63]:'')}return output}
async function generateE2EEKey(){return base64Url(await Crypto.getRandomBytesAsync(24))}
async function e2eeFingerprint(key:string){if(!E2EE_KEY_PATTERN.test(key))return'';const digest=await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256,key);return `${digest.slice(0,4)}-${digest.slice(4,8)}-${digest.slice(8,12)}`.toUpperCase()}

type MeetingConnectionProps={auth:Auth;e2eeKey:string;mic:boolean;camera:boolean;onLeave:()=>void;theme:ThemeName;setTheme:(theme:ThemeName)=>void};
const roomOptions={adaptiveStream:{pixelDensity:'screen' as const},dynacast:true};
function PlainMeetingConnection({auth,mic,camera,onLeave,theme,setTheme}:MeetingConnectionProps){return <LiveKitRoom serverUrl={auth.url} token={auth.token} connect audio={mic?{echoCancellation:true,noiseSuppression:true,autoGainControl:true}:false} video={camera} options={roomOptions} onDisconnected={onLeave} onError={e=>Alert.alert('Reunião',errorMessage(e))}><Meeting auth={auth} onLeave={onLeave} theme={theme} setTheme={setTheme}/></LiveKitRoom>}
function EncryptedMeetingConnection({auth,e2eeKey,mic,camera,onLeave,theme,setTheme}:MeetingConnectionProps){
  const {e2eeManager}=useRNE2EEManager({sharedKey:e2eeKey,keyProviderOptions:{sharedKey:true,ratchetSalt:'LKFrameEncryptionKey',ratchetWindowSize:16,failureTolerance:-1,keyringSize:16,keySize:128}});
  return <LiveKitRoom serverUrl={auth.url} token={auth.token} connect audio={mic?{echoCancellation:true,noiseSuppression:true,autoGainControl:true}:false} video={camera} options={{...roomOptions,e2ee:{e2eeManager}}} onDisconnected={onLeave} onEncryptionError={()=>{Alert.alert('Chave incorreta','A mídia criptografada não pôde ser decifrada. Confira a chave da reunião.');onLeave()}} onError={e=>Alert.alert('Reunião',errorMessage(e))}><Meeting auth={auth} onLeave={onLeave} theme={theme} setTheme={setTheme}/></LiveKitRoom>
}
...
function App() {
  ...
  const [e2eeEnabled,setE2eeEnabled]=useState(false),[e2eeKey,setE2eeKey]=useState(''),[e2eePrint,setE2eePrint]=useState('');
  ...
  useEffect(()=>{let active=true;if(!e2eeEnabled||!E2EE_KEY_PATTERN.test(e2eeKey)){setE2eePrint('');return()=>{active=false}}void e2eeFingerprint(e2eeKey).then(value=>{if(active)setE2eePrint(value)});return()=>{active=false}},[e2eeEnabled,e2eeKey]);
  useEffect(()=>{
  ...
  const reset=useCallback(()=>{operation.current++;stopPreview.current();setRequest(null);setAuth(null);setE2eeEnabled(false);setE2eeKey('');setE2eePrint('');setScreen('home');setBusy(false);setMic(false);setCamera(false);void AudioSession.stopAudioSession().catch(()=>{})},[]);
  useEffect(()=>{
    ...
    const poll=async()=>{
      try {
        ...
        if(result.status==='approved'){if(!!result.e2ee!==e2eeEnabled||result.e2ee&&!E2EE_KEY_PATTERN.test(e2eeKey))throw new Error('O modo ou a chave de criptografia não corresponde a esta reunião. A conexão foi bloqueada.');await AudioSession.startAudioSession();if(!active){void AudioSession.stopAudioSession();return}setAuth(result);setScreen('meeting');return}
        setError('');
  ...
  },[request,screen,reset,e2eeEnabled,e2eeKey]);
  async function enter(){
    ...
    try {
      ...
      if(e2eeEnabled&&!E2EE_KEY_PATTERN.test(e2eeKey))throw new Error('Informe a chave de 32 caracteres da reunião criptografada.');
      const body={...profile,name:profile.name.trim(),e2ee:e2eeEnabled};
      if(mode==='create'){
        ...
        if(!!result.e2ee!==e2eeEnabled)throw new Error('O modo de criptografia recebido não corresponde ao escolhido. A conexão foi bloqueada.');setAuth(result);setScreen('meeting');
      }else{
  ...
  async function changeE2EE(value:boolean){
    setError('');setE2eeEnabled(value);setE2eePrint('');
    if(!value){setE2eeKey('');return}
    if(mode==='create'){try{setE2eeKey(await generateE2EEKey())}catch{setE2eeEnabled(false);setError('Este aparelho não conseguiu gerar uma chave segura.')}}else setE2eeKey('');
  }
  ...
  return <Theme.Provider value={c}><SafeAreaView style={[s.root,{backgroundColor:c.bg}]}><KeyboardAvoidingView style={s.root} behavior={Platform.OS==='ios'?'padding':'height'}><StatusBar style={darkMode?'light':'dark'}/>
    {screen==='meeting'&&auth ? auth.e2ee?<EncryptedMeetingConnection auth={auth} e2eeKey={e2eeKey} mic={mic} camera={camera} onLeave={reset} theme={theme} setTheme={setTheme}/>:<PlainMeetingConnection auth={auth} e2eeKey="" mic={mic} camera={camera} onLeave={reset} theme={theme} setTheme={setTheme}/> : <ScrollView ref={homeScroll} contentContainerStyle={s.page} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
      ...
      {screen==='waiting' ? <View style={[s.waitingCard,{backgroundColor:c.surface,borderColor:c.border}]}><Ionicons name="hourglass" size={54} color={c.accent}/><Text style={[s.title,{color:c.text}]}>Aguardando o anfitrião</Text><Text style={{color:c.muted}}>Sua solicitação foi enviada. Você entrará quando ela for aceita.</Text><Text style={[s.roomCode,{color:c.accent,borderColor:c.border}]}>{request?.room}</Text><Button label="Cancelar" icon="close" onPress={reset}/></View> : <>
      ...
      {screen==='home'?<View style={[s.heroCard,{backgroundColor:c.surface,borderColor:c.border}]}><View style={[s.heroGlow,{backgroundColor:c.accent+'24'}]}/><Text style={[s.kicker,{color:c.accent}]}>CONVERSAS QUE APROXIMAM</Text><Text style={[s.title,{color:c.text}]}>Reuniões simples. Conexões reais.</Text><Text style={[s.heroLead,{color:c.muted}]}>Vídeo, voz e tela em um só lugar, com uma experiência feita para o seu aparelho.</Text></View>:<Text style={[s.title,{color:c.text}]}>Como você quer entrar?</Text>}
      ...
      {screen==='home'?<View style={s.section}><Button label="Nova reunião" icon="add-circle" onPress={()=>{setMode('create');setError('');setScreen('preview')}}/><TextInput accessibilityLabel="Código ou link da reunião" placeholder="Código ou link da reunião" placeholderTextColor={c.muted} value={code} onChangeText={setCode} onFocus={()=>setTimeout(()=>homeScroll.current?.scrollToEnd({animated:true}),200)} autoCapitalize="none" autoCorrect={false} style={[s.input,{color:c.text,borderColor:c.border}]}/><Button label="Participar" icon="enter" disabled={!roomCode(code)} onPress={()=>{setMode('join');setError('');setScreen('preview')}}/><Text style={{color:c.muted}}>Windows, navegador, Android e iPhone nas mesmas salas.</Text></View>:<View style={s.section}>
        ...
        <View style={[s.e2eeCard,{backgroundColor:c.surface,borderColor:c.border}]}><View style={s.setting}><Ionicons name="shield-checkmark" size={24} color={c.text}/><View style={{flex:1,gap:3}}><Text style={{color:c.text,fontWeight:'700'}}>Mídia com E2EE · experimental</Text><Text style={{color:c.muted,fontSize:12}}>Criptografa áudio, câmera e tela. O chat continua protegido em trânsito.</Text></View><Switch accessibilityLabel="Ativar mídia com criptografia ponta a ponta" value={e2eeEnabled} onValueChange={value=>void changeE2EE(value)}/></View>{e2eeEnabled&&<View style={{gap:9}}><TextInput accessibilityLabel="Chave da reunião criptografada" placeholder="Chave de 32 caracteres" placeholderTextColor={c.muted} maxLength={32} autoCapitalize="none" autoCorrect={false} secureTextEntry={mode==='join'} selectTextOnFocus value={e2eeKey} onChangeText={value=>setE2eeKey(value.trim())} style={[s.input,s.e2eeInput,{color:c.text,borderColor:c.border}]}/>{mode==='create'&&<Button label="Gerar outra chave segura" icon="refresh" onPress={()=>void generateE2EEKey().then(setE2eeKey).catch(()=>setError('Este aparelho não conseguiu gerar uma chave segura.'))}/>} {!!e2eePrint&&<Text selectable accessibilityRole="text" style={{color:c.accent,fontWeight:'700'}}>Impressão para conferência: {e2eePrint}</Text>}<Text style={{color:c.muted,fontSize:12,lineHeight:18}}>A chave fica somente na memória deste aparelho. Compartilhe-a separadamente; ela não vai no convite.</Text></View>}</View>
...
function Meeting({auth,onLeave,theme,setTheme}:{auth:Auth;onLeave:()=>void;theme:ThemeName;setTheme:(theme:ThemeName)=>void}) {
  ...
  const memberKey=auth.memberKey||auth.hostKey||'';
  const [adminRole,setAdminRole]=useState<AdminRole>(auth.host?'host':'participant'),[adminKey,setAdminKey]=useState(auth.hostKey||'');
  const [roles,setRoles]=useState<Record<string,AdminRole>>({}),[permissions,setPermissionsState]=useState<MemberPermissions>({...DEFAULT_MEMBER_PERMISSIONS}),[memberPermissions,setMemberPermissions]=useState<Record<string,MemberPermissions>>({});
  const isHost=adminRole==='host',isAdmin=isHost||adminRole==='cohost';
  const [messages,setMessages]=useState<Message[]>([]),[draft,setDraft]=useState(''),[replyTo,setReplyTo]=useState<Message|null>(null);
  const [emojiOpen,setEmojiOpen]=useState(false),[emojiCategory,setEmojiCategory]=useState<EmojiCategory>('rostos'),[emojiSearch,setEmojiSearch]=useState(''),[recentEmojis,setRecentEmojis]=useState<string[]>([]);
  const [raised,setRaised]=useState<Set<string>>(new Set()),[busy,setBusy]=useState(false),[error,setError]=useState('');
  const [moderationEvents,setModerationEvents]=useState<ModerationEvent[]>([]),[adminNotice,setAdminNotice]=useState('');
  ...
  const addMessage=useCallback((message:Message,unreadMessage=false)=>{setMessages(items=>items.some(item=>item.id===message.id)?items:[...items,message].slice(-200));if(unreadMessage&&panelRef.current!=='chat')setUnread(n=>n+1)},[]);
  const addSystem=useCallback((text:string)=>addMessage({id:`system-${Date.now()}-${Math.random()}`,name:'Kpnc Meet',text,system:true,createdAt:Date.now()}),[addMessage]);
  useEffect(()=>{
  ...
  useEffect(()=>{void Promise.all([AsyncStorage.getItem('kpnc-share-quality'),AsyncStorage.getItem('kpnc-recent-emojis')]).then(([qualityValue,emojiValue])=>{
    try {
      const saved=JSON.parse(qualityValue||'{}') as {resolution?:ShareResolution;fps?:ShareFps};
    ...
    try{const saved=JSON.parse(emojiValue||'[]');if(Array.isArray(saved))setRecentEmojis(saved.filter(value=>typeof value==='string').slice(0,24))}catch{}
  })},[]);
  useEffect(()=>{
    ...
    const data=(payload:Uint8Array,p:Participant|undefined,_kind:unknown,topic?:string)=>{
      ...
      try {
        ...
        if(topic==='chat'&&typeof d.text==='string')addMessage({id:String(d.id||`${Date.now()}-${Math.random()}`).slice(0,80),name:p.name||'Participante',identity:p.identity,text:d.text.slice(0,500),createdAt:Number(d.createdAt)||Date.now(),replyTo:d.replyTo},true);
    ...
    const joined=(p:Participant)=>addSystem(`${p.name||'Participante'} entrou na reunião.`),left=(p:Participant)=>addSystem(`${p.name||'Participante'} saiu da reunião.`);room.on(RoomEvent.ParticipantConnected,joined);room.on(RoomEvent.ParticipantDisconnected,left);
    return()=>{for(const event of events)room.off(event,update);room.off(RoomEvent.ParticipantConnected,joined);room.off(RoomEvent.ParticipantDisconnected,left);room.off(RoomEvent.DataReceived,data);room.off(RoomEvent.ConnectionStateChanged,setConnection);room.off(RoomEvent.ConnectionQualityChanged,qualityChanged);room.off(RoomEvent.ActiveSpeakersChanged,active)};
  },[room,onLeave,addMessage,addSystem]);
  useEffect(()=>{
    if(!memberKey)return;
    let active=true;let timer:ReturnType<typeof setTimeout>;const controller=new AbortController();
    const poll=async()=>{
      try{const result=await api<RoleStatus>(`/api/rooms/${auth.room}/role`,{headers:{Authorization:`Bearer ${memberKey}`},signal:controller.signal});if(active){setAdminRole(result.role);setAdminKey(result.role==='host'||result.role==='cohost'?memberKey:'');setRoles(result.roles||{});setPermissionsState(result.permissions||{...DEFAULT_MEMBER_PERMISSIONS});setMemberPermissions(result.memberPermissions||{});setLocked(!!result.locked)}}
  ...
  useEffect(()=>{
    ...
    let active=true;let timer:ReturnType<typeof setTimeout>;const controller=new AbortController();
    const poll=async()=>{
      try{const result=await api<{messages:Message[]}>(`/api/rooms/${auth.room}/messages`,{headers:{Authorization:`Bearer ${memberKey}`},signal:controller.signal});if(active)for(const message of result.messages||[])addMessage(message)}
      catch(error){if(active)console.warn('CHAT_HISTORY_FAILED',errorMessage(error))}
      if(active)timer=setTimeout(poll,1800);
    };
    void poll();return()=>{active=false;controller.abort();clearTimeout(timer)};
  ...
  useEffect(()=>{
    ...
    let active=true;let timer:ReturnType<typeof setTimeout>;const controller=new AbortController();
    const poll=async()=>{
      try{const result=await api<{requests:Pending[]}>(`/api/rooms/${auth.room}/requests`,{headers:{Authorization:`Bearer ${adminKey}`},signal:controller.signal});if(active)setPending(result.requests)}
      catch(error){if(active)setError(errorMessage(error))}
      if(active)timer=setTimeout(poll,1800);
    };
    void poll();return()=>{active=false;controller.abort();clearTimeout(timer);setPending([])};
  },[auth.room,isAdmin,adminKey]);
  useEffect(()=>{
    if(!isAdmin||!adminKey){setModerationEvents([]);setAdminNotice('');return}
    let active=true;let timer:ReturnType<typeof setTimeout>;const controller=new AbortController();
    const poll=async()=>{
      try{const result=await api<{events:ModerationEvent[]}>(`/api/rooms/${auth.room}/moderation-history`,{headers:{Authorization:`Bearer ${adminKey}`},signal:controller.signal});if(active)setModerationEvents((result.events||[]).slice(-100))}
      catch(error){if(active)console.warn('MODERATION_HISTORY_FAILED',errorMessage(error))}
  ...
  async function adminAct(pendingLabel:string,successLabel:string,fn:()=>Promise<unknown>){setAdminNotice(pendingLabel);try{await fn();setAdminNotice(successLabel)}catch(error){setAdminNotice(errorMessage(error));throw error}}
  ...
  async function decide(id:string,decision:'admit'|'deny') {await adminAct(decision==='admit'?'Autorizando entrada…':'Recusando solicitação…',decision==='admit'?'Entrada autorizada.':'Solicitação recusada.',async()=>{await api(`/api/rooms/${auth.room}/requests/${id}/${decision}`,post({},adminKey));setPending(items=>items.filter(item=>item.id!==id))})}
  async function toggleLock(){const next=!locked;await adminAct(next?'Bloqueando novas entradas…':'Liberando novas entradas…',next?'Reunião bloqueada para novas entradas.':'Novas entradas permitidas.',async()=>{const result=await api<{locked:boolean}>(`/api/rooms/${auth.room}/settings`,post({locked:next},adminKey));setLocked(result.locked)})}
  async function muteAll(){const targets=participants.filter(p=>p!==room.localParticipant).map(p=>({p,track:p.getTrackPublication(Track.Source.Microphone)})).filter(item=>item.track&&!item.track.isMuted);if(!targets.length)throw new Error('Todos já estão com o microfone desligado.');await adminAct('Silenciando participantes…','Todos os microfones disponíveis foram desativados.',()=>Promise.all(targets.map(({p,track})=>api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/mute`,post({trackSid:track?.trackSid},adminKey)))))}
  async function changeRole(p:Participant,role:'participant'|'cohost'){await adminAct('Alterando função…',role==='cohost'?'Coanfitrião adicionado.':'Coanfitrião removido.',()=>api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/role`,post({role},adminKey)))}
  async function transferHost(p:Participant){await adminAct('Transferindo função de anfitrião…','Função de anfitrião transferida.',()=>api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/transfer`,post({},adminKey)))}
  async function updatePermissions(p:Participant,key:keyof MemberPermissions){const current=memberPermissions[p.identity]||DEFAULT_MEMBER_PERMISSIONS;const next={...current,[key]:!current[key]},label={microphone:'Microfone',camera:'Câmera',chat:'Chat',screen:'Compartilhamento'}[key];await adminAct('Atualizando permissão…',`${label} ${next[key]?'permitido':'bloqueado'} para ${p.name||'participante'}.`,async()=>{await api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/permissions`,post({permissions:next},adminKey));setMemberPermissions(values=>({...values,[p.identity]:next}))})}
  async function muteParticipant(p:Participant){const trackSid=p.getTrackPublication(Track.Source.Microphone)?.trackSid;if(!trackSid)throw new Error('Este participante já está com o microfone desligado.');await adminAct('Desativando microfone…','Microfone desativado.',()=>api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/mute`,post({trackSid},adminKey)))}
  async function removeParticipant(p:Participant){await adminAct('Removendo participante…','Participante removido.',()=>api(`/api/rooms/${auth.room}/participants/${encodeURIComponent(p.identity)}/remove`,post({},adminKey)))}
  async function send(){if(!permissions.chat)throw new Error('O chat foi bloqueado pelo anfitrião.');const text=draft.trim().slice(0,500);if(!text||!memberKey)return;const id=`${Date.now()}-${Math.random().toString(36).slice(2)}`;const result=await api<{message:Message}>(`/api/rooms/${auth.room}/messages`,post({id,name:room.localParticipant.name||'Participante',text,replyId:replyTo?.id||''},memberKey));addMessage(result.message);await publish('chat',result.message);setDraft('');setReplyTo(null)}
  function chooseEmoji(emoji:string){setDraft(value=>(value+emoji).slice(0,500));const next=[emoji,...recentEmojis.filter(item=>item!==emoji)].slice(0,24);setRecentEmojis(next);void AsyncStorage.setItem('kpnc-recent-emojis',JSON.stringify(next));setEmojiOpen(false)}
  ...
  async function startScreenShare(){
    if(!permissions.screen)throw new Error('O compartilhamento de tela foi bloqueado pelo anfitrião.');
  ...
  const tileKeys=tiles.map(item=>item.key).join('|');
  const focusedTile=tiles.find(item=>item.key===focused);
  const tileSourceLabel=(item:Tile)=>item.track?.source===Track.Source.ScreenShare?'Tela':item.track?.source===Track.Source.Camera?'Câmera':'Sem vídeo';
  const selectViewerTile=(key:string)=>{setFocused(key);setFitMode('contain');setZoom(1)};
  useEffect(()=>{
    if(focused===null||focusedTile)return;
    const next=tiles.find(item=>item.track?.source===Track.Source.ScreenShare)||tiles.find(item=>item.track?.source===Track.Source.Camera)||tiles[0];
    if(next)selectViewerTile(next.key);else closeViewer();
  },[focused,tileKeys]);
  const tile=(item:(typeof tiles)[number],fullscreen=false)=><View key={item.key} style={[s.tile,{backgroundColor:c.surface,borderColor:speakers.has(item.participant.identity)?'#32d583':c.border,width:fullscreen?'100%':wide?'48.5%':'100%',height:fullscreen?undefined:(wide?Math.max(150,height*.5):Math.min(310,width*.7))},fullscreen&&s.fullTile]}>
    {item.track?<VideoTrack trackRef={item.track} style={fullscreen&&zoom!==1?{position:'absolute',top:0,right:0,bottom:0,left:0,transform:[{scale:zoom}]}:StyleSheet.absoluteFill} objectFit={item.track.source===Track.Source.ScreenShare?(fullscreen?fitMode:'contain'):'cover'}/>:<Avatar profile={participantProfile(item.participant)}/>}
  ...
  const messageBubble=(item:Message)=><View key={item.id} accessibilityRole="text" accessibilityLabel={item.system?item.text:`Mensagem de ${item.name}`} style={[s.bubble,item.system?s.systemMessage:{backgroundColor:c.surface}]}>
    {!!item.replyTo&&<View style={[s.replyQuote,{borderColor:c.accent}]}><Text numberOfLines={1} style={{color:c.muted,fontSize:11}}>{item.replyTo.name}: {item.replyTo.text}</Text></View>}
    {!item.system&&<View style={s.messageHeader}><Text style={{color:c.accent,fontWeight:'700'}}>{item.identity===room.localParticipant.identity?'Você':item.name}</Text><Text style={{color:c.muted,fontSize:10}}>{new Date(item.createdAt||Date.now()).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</Text></View>}
    <MessageText text={item.text} color={item.system?c.muted:c.text}/>
    {!item.system&&<Pressable accessibilityRole="button" accessibilityLabel={`Responder a ${item.name}`} onPress={()=>setReplyTo(item)}><Text style={{color:c.accent,fontSize:12,fontWeight:'700'}}>Responder</Text></Pressable>}
  </View>;
  const emojiItems=(emojiCategory==='recentes'?recentEmojis:MOBILE_EMOJIS[emojiCategory] as readonly string[]).filter(emoji=>!emojiSearch.trim()||emoji.includes(emojiSearch.trim()));
  const connectionText=(auth.e2ee?'E2EE · ':'')+(!connected?(connection==='reconnecting'?'Reconectando…':'Conectando…'):({excellent:'Conexão ótima',good:'Conexão boa',poor:'Conexão instável',lost:'Conexão perdida'} as Record<string,string>)[quality]||'Conectado');
  const participantName=(identity?:string)=>participants.find(p=>p.identity===identity)?.name||'Participante';
  const moderationText=(event:ModerationEvent)=>{
    const actor=event.actorIdentity===room.localParticipant.identity?'Você':participantName(event.actorIdentity),target=event.targetName||participantName(event.targetIdentity);
    const labels:Record<string,string>={admit:`autorizou a entrada de ${target}`,deny:`recusou a entrada de ${target}`,lock:'bloqueou novas entradas',unlock:'permitiu novas entradas',mute:`silenciou ${target}`,remove:`removeu ${target}`,promote_cohost:`tornou ${target} coanfitrião`,demote_cohost:`removeu ${target} da função de coanfitrião`,transfer_host:`transferiu a função de anfitrião para ${target}`,end_room:'encerrou a reunião'};
    if(event.action==='permissions'){const names:Record<string,string>={microphone:'microfone',camera:'câmera',chat:'chat',screen:'compartilhamento'},changes=Object.entries(event.changes||{}).map(([key,value])=>`${names[key]||key}: ${value?'permitido':'bloqueado'}`).join(', ');return `${actor} atualizou ${target} — ${changes||'permissões'}`}
  ...
  return <View style={s.root}><StatusBar hidden={focused!==null} style="light"/>
    ...
    <View style={[s.primaryControls,{borderColor:c.border,backgroundColor:c.surface}]}>
      {([
        [room.localParticipant.isMicrophoneEnabled?'mic':'mic-off','Mic',()=>void act(()=>room.localParticipant.setMicrophoneEnabled(!room.localParticipant.isMicrophoneEnabled,{echoCancellation:true,noiseSuppression:true,autoGainControl:true})),busy||!connected||!permissions.microphone],
        [room.localParticipant.isCameraEnabled?'videocam':'videocam-off','Câmera',()=>void act(()=>room.localParticipant.setCameraEnabled(!room.localParticipant.isCameraEnabled)),busy||!connected||!permissions.camera],
    ...
    <Modal visible={toolsOpen} animationType="slide" onRequestClose={()=>setToolsOpen(false)}><SafeAreaView style={[s.root,{backgroundColor:c.bg}]}><ScrollView contentContainerStyle={s.page}><Text style={[s.title,{color:c.text}]}>Na sua reunião</Text>
      <Button label={room.localParticipant.isScreenShareEnabled?'Parar apresentação':'Compartilhar tela'} icon="easel" disabled={busy||!connected||(!permissions.screen&&!room.localParticipant.isScreenShareEnabled)} onPress={()=>{setToolsOpen(false);if(room.localParticipant.isScreenShareEnabled)void act(()=>room.localParticipant.setScreenShareEnabled(false));else setShareSettingsOpen(true)}}/>
    ...
    <Modal visible={focused!==null} animationType="fade" statusBarTranslucent navigationBarTranslucent onRequestClose={closeViewer} supportedOrientations={['portrait','landscape']}><SafeAreaView style={[s.root,{backgroundColor:'#000'}]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} accessibilityLabel="Escolher participante ou transmissão" contentContainerStyle={s.viewerParticipants} style={s.viewerParticipantsScroll}>
        {tiles.map(item=>{const selected=item.key===focused;return <Pressable key={item.key} accessibilityRole="button" accessibilityLabel={`Mostrar ${item.participant.name||'Participante'} — ${tileSourceLabel(item)}`} accessibilityState={{selected}} onPress={()=>selectViewerTile(item.key)} style={[s.viewerParticipant,{borderColor:selected?c.accent:'#ffffff35',backgroundColor:selected?c.accent+'35':'#ffffff12'}]}><Avatar profile={participantProfile(item.participant)} size={30}/><View style={{maxWidth:150}}><Text numberOfLines={1} style={s.viewerParticipantName}>{item.participant.name||'Participante'}{item.participant===room.localParticipant?' (você)':''}</Text><Text style={s.viewerParticipantSource}>{tileSourceLabel(item)}</Text></View>{selected&&<Ionicons name="checkmark-circle" color={c.accent} size={19}/>}</Pressable>})}
      </ScrollView>
      <View style={[s.viewerBody,{flexDirection:wide?'row':'column'}]}>
        <View style={s.viewerStage}>{focusedTile?tile(focusedTile,true):<Text style={{color:'white',padding:24}}>A transmissão terminou.</Text>}</View>
        {viewerMode==='chat'&&<KeyboardAvoidingView style={[s.viewerChat,{backgroundColor:c.bg}]} behavior={Platform.OS==='ios'?'padding':'height'}><ScrollView contentContainerStyle={s.viewerMessages} keyboardShouldPersistTaps="handled">{messages.length===0&&<Text style={{color:c.muted}}>As mensagens aparecem aqui durante a reunião.</Text>}{messages.map(messageBubble)}</ScrollView><View style={s.composerWrap}>{!!replyTo&&<View style={[s.replyPreview,{borderColor:c.accent}]}><Text numberOfLines={1} style={{color:c.muted,flex:1}}>Respondendo a {replyTo.name}: {replyTo.text}</Text><Pressable accessibilityRole="button" accessibilityLabel="Cancelar resposta" onPress={()=>setReplyTo(null)}><Ionicons name="close" color={c.muted} size={20}/></Pressable></View>}<View style={s.viewerComposer}><Pressable accessibilityRole="button" accessibilityLabel="Escolher emoji" onPress={()=>setEmojiOpen(true)} style={s.emojiButton}><Text style={{fontSize:24}}>☺</Text></Pressable><TextInput accessibilityLabel="Mensagem no modo imersivo" placeholder={permissions.chat?'Escreva uma mensagem':'Chat bloqueado pelo anfitrião'} placeholderTextColor={c.muted} editable={permissions.chat} value={draft} onChangeText={setDraft} maxLength={500} style={[s.input,{color:c.text,borderColor:c.border,flex:1}]} onSubmitEditing={()=>void act(send)}/><Pressable accessibilityRole="button" accessibilityLabel="Enviar mensagem" disabled={busy||!draft.trim()||!connected||!permissions.chat} onPress={()=>void act(send)} style={[s.viewerSend,{backgroundColor:c.accent,opacity:(busy||!draft.trim()||!connected||!permissions.chat) ? .4 : 1}]}><Ionicons name="send" color="white" size={21}/></Pressable></View></View></KeyboardAvoidingView>}
      </View>
    </SafeAreaView></Modal>
    ...
    <Modal visible={panel!==null} animationType="slide" onRequestClose={()=>setPanel(null)}><SafeAreaView style={[s.root,{backgroundColor:c.bg}]}><KeyboardAvoidingView style={s.root} behavior={Platform.OS==='ios'?'padding':'height'}><View style={s.meetingHeader}><Text style={[s.brand,{color:c.text,flex:1}]}>{panel==='chat'?'Chat da reunião':'Participantes'}</Text><Button label="Fechar" icon="close" onPress={()=>setPanel(null)}/></View>
      {panel==='chat'?<><ScrollView contentContainerStyle={s.section} keyboardShouldPersistTaps="handled">{messages.length===0&&<Text style={{color:c.muted}}>As mensagens aparecem aqui durante a reunião.</Text>}{messages.map(messageBubble)}</ScrollView><View style={s.composerWrap}>{!!replyTo&&<View style={[s.replyPreview,{borderColor:c.accent}]}><Text numberOfLines={1} style={{color:c.muted,flex:1}}>Respondendo a {replyTo.name}: {replyTo.text}</Text><Pressable accessibilityRole="button" accessibilityLabel="Cancelar resposta" onPress={()=>setReplyTo(null)}><Ionicons name="close" color={c.muted} size={20}/></Pressable></View>}<View style={s.composer}><Pressable accessibilityRole="button" accessibilityLabel="Escolher emoji" onPress={()=>setEmojiOpen(true)} style={s.emojiButton}><Text style={{fontSize:24}}>☺</Text></Pressable><TextInput accessibilityLabel="Mensagem" placeholder={permissions.chat?'Escreva uma mensagem':'Chat bloqueado pelo anfitrião'} placeholderTextColor={c.muted} editable={permissions.chat} value={draft} onChangeText={setDraft} maxLength={500} style={[s.input,{color:c.text,borderColor:c.border,flex:1}]} onSubmitEditing={()=>void act(send)}/><Button label="Enviar" icon="send" disabled={busy||!draft.trim()||!connected||!permissions.chat} onPress={()=>void act(send)}/></View></View></>:<ScrollView contentContainerStyle={s.section}>
        {isAdmin&&<><Text style={[s.subtitle,{color:c.text}]}>Aguardando para entrar ({pending.length})</Text>{pending.map(p=><View key={p.id} style={[s.bubble,{backgroundColor:c.surface}]}><Text style={{color:c.text}}>{p.name}</Text><View style={s.row}><Button label="Aceitar" icon="checkmark" disabled={busy} onPress={()=>Alert.alert('Permitir entrada?',`Autorizar ${p.name} a entrar na reunião?`,[{text:'Cancelar',style:'cancel'},{text:'Permitir',onPress:()=>void act(()=>decide(p.id,'admit'))}])}/><Button label="Recusar" icon="close" disabled={busy} onPress={()=>Alert.alert('Recusar entrada?',`Recusar a solicitação de ${p.name}?`,[{text:'Cancelar',style:'cancel'},{text:'Recusar',style:'destructive',onPress:()=>void act(()=>decide(p.id,'deny'))}])}/></View></View>)}</>}
        <Text style={[s.subtitle,{color:c.text}]}>Na reunião ({participants.length})</Text>{participants.map(p=>{const role=p===room.localParticipant?adminRole:(roles[p.identity]||'participant'),current=memberPermissions[p.identity]||DEFAULT_MEMBER_PERMISSIONS;return <View key={p.identity} style={[s.bubble,{backgroundColor:c.surface}]}><View style={s.row}><Avatar profile={participantProfile(p)} size={40}/><Text style={{color:c.text,flex:1}}>{p.name||'Participante'}{p===room.localParticipant?' (você)':''}{role==='host'?' · ♛':role==='cohost'?' · ◆':''}{raised.has(p.identity)?' · ✋':''}</Text><Ionicons name={p.isMicrophoneEnabled?'mic':'mic-off'} size={20} color={c.muted}/></View>{isAdmin&&p!==room.localParticipant&&<><View style={s.row}><Button label="Silenciar" icon="mic-off" disabled={busy||!p.isMicrophoneEnabled} onPress={()=>Alert.alert('Desativar microfone?',`Silenciar ${p.name||'este participante'}?`,[{text:'Cancelar',style:'cancel'},{text:'Silenciar',onPress:()=>void act(()=>muteParticipant(p))}])}/><Button label="Remover" icon="person-remove" disabled={busy} onPress={()=>Alert.alert('Remover participante?',p.name||'Participante',[{text:'Cancelar',style:'cancel'},{text:'Remover',style:'destructive',onPress:()=>void act(()=>removeParticipant(p))}])}/>{isHost&&<Button label={role==='cohost'?'Remover coanfitrião':'Tornar coanfitrião'} icon="shield-checkmark" disabled={busy} onPress={()=>Alert.alert('Alterar função?',p.name||'Participante',[{text:'Cancelar',style:'cancel'},{text:'Confirmar',onPress:()=>void act(()=>changeRole(p,role==='cohost'?'participant':'cohost'))}])}/>} {isHost&&<Button label="Transferir anfitrião" icon="key" disabled={busy} onPress={()=>Alert.alert('Transferir anfitrião?',`${p.name||'Participante'} será o anfitrião principal e você continuará como coanfitrião.`,[{text:'Cancelar',style:'cancel'},{text:'Transferir',style:'destructive',onPress:()=>void act(()=>transferHost(p))}])}/>}</View><Text style={{color:c.muted,fontSize:12}}>Permissões individuais</Text><View style={s.permissionRow}>{(['microphone','camera','chat','screen'] as (keyof MemberPermissions)[]).map(key=>{const label={microphone:'microfone',camera:'câmera',chat:'chat',screen:'compartilhamento'}[key],verb=current[key]?'Bloquear':'Permitir';return <Pressable key={key} disabled={busy} accessibilityRole="button" accessibilityState={{selected:current[key],disabled:busy}} onPress={()=>Alert.alert('Alterar permissão?',`${verb} ${label} para ${p.name||'este participante'}?`,[{text:'Cancelar',style:'cancel'},{text:'Confirmar',onPress:()=>void act(()=>updatePermissions(p,key))}])} style={[s.permissionChip,{opacity:busy?.45:1,borderColor:current[key]?c.accent:c.border,backgroundColor:current[key]?c.accent+'22':c.bg}]}><Text style={{color:c.text,fontSize:12,fontWeight:'700'}}>{{microphone:'Mic',camera:'Câmera',chat:'Chat',screen:'Tela'}[key]}</Text></Pressable>})}</View></>}</View>})}
        {isAdmin&&<><View style={s.row}><Button label={locked?'Permitir novas entradas':'Bloquear novas entradas'} icon={locked?'lock-open':'lock-closed'} disabled={busy} onPress={()=>Alert.alert(locked?'Permitir novas entradas?':'Bloquear novas entradas?',locked?'Novas pessoas poderão solicitar entrada novamente.':'Novas solicitações serão recusadas enquanto a sala estiver bloqueada.',[{text:'Cancelar',style:'cancel'},{text:'Confirmar',onPress:()=>void act(toggleLock)}])}/><Button label="Silenciar todos" icon="mic-off" disabled={busy} onPress={()=>Alert.alert('Silenciar todos?','Os outros participantes ficarão com o microfone desligado.',[{text:'Cancelar',style:'cancel'},{text:'Silenciar',onPress:()=>void act(muteAll)}])}/></View>{isHost&&<Button label="Encerrar para todos" icon="stop-circle" danger disabled={busy} onPress={end}/>} {!!adminNotice&&<View accessibilityRole="alert" style={[s.adminNotice,{backgroundColor:c.surface,borderColor:c.border}]}><Ionicons name={busy?'hourglass-outline':'checkmark-circle'} size={19} color={busy?c.muted:'#32d583'}/><Text style={{color:c.text,flex:1}}>{adminNotice}</Text></View>}<Text style={[s.subtitle,{color:c.text}]}>Atividade da moderação</Text>{moderationEvents.length?moderationEvents.slice(-30).reverse().map(event=><View key={event.id} style={[s.moderationEvent,{borderColor:c.border}]}><Text style={{color:c.text,flex:1,fontSize:13}}>{moderationText(event)}</Text><Text style={{color:c.muted,fontSize:10}}>{new Date(event.createdAt).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</Text></View>):<Text style={{color:c.muted}}>As ações administrativas aparecerão aqui.</Text>}</>}
      </ScrollView>}
...
const s=StyleSheet.create({
  root:{flex:1},page:{padding:18,gap:18,paddingBottom:34,width:'100%',maxWidth:780,alignSelf:'center'},header:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12,paddingVertical:4},brand:{fontSize:21,fontWeight:'800',letterSpacing:-.5},title:{fontSize:30,fontWeight:'800',lineHeight:36,letterSpacing:-.7},subtitle:{fontSize:20,fontWeight:'700'},section:{padding:18,gap:16},row:{flexDirection:'row',alignItems:'center',gap:10,flexWrap:'wrap'},heroCard:{position:'relative',overflow:'hidden',borderWidth:1,borderRadius:28,padding:24,gap:9,shadowColor:'#18345f',shadowOpacity:.13,shadowRadius:24,shadowOffset:{width:0,height:12},elevation:7},heroGlow:{position:'absolute',width:190,height:190,borderRadius:95,right:-90,top:-105},kicker:{fontSize:11,fontWeight:'800',letterSpacing:2},heroLead:{fontSize:15,lineHeight:23,maxWidth:560},waitingCard:{marginTop:48,borderWidth:1,borderRadius:28,padding:26,gap:16,alignItems:'center',shadowColor:'#18345f',shadowOpacity:.13,shadowRadius:24,shadowOffset:{width:0,height:12},elevation:7},roomCode:{borderWidth:1,borderRadius:999,paddingHorizontal:13,paddingVertical:7,fontWeight:'700',letterSpacing:.8},profile:{borderWidth:1,borderRadius:28,padding:22,gap:16,alignItems:'center',shadowColor:'#18345f',shadowOpacity:.1,shadowRadius:20,shadowOffset:{width:0,height:10},elevation:5},avatar:{backgroundColor:'#308ee3',alignItems:'center',justifyContent:'center'},input:{borderWidth:1,borderRadius:16,padding:16,fontSize:17,minHeight:54,width:'100%'},e2eeCard:{borderWidth:1,borderRadius:20,padding:14,gap:11},e2eeInput:{fontFamily:Platform.select({ios:'Menlo',android:'monospace'}),letterSpacing:1},button:{minHeight:48,padding:12,borderRadius:16,borderWidth:1,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:7,shadowColor:'#18345f',shadowOpacity:.06,shadowRadius:9,shadowOffset:{width:0,height:4},elevation:2},buttonText:{fontSize:13,fontWeight:'600'},setting:{flexDirection:'row',alignItems:'center',gap:12,minHeight:52,paddingHorizontal:4},checks:{gap:9,padding:14,borderRadius:20},deviceCheck:{minHeight:58,padding:12,borderWidth:1,borderRadius:16,flexDirection:'row',alignItems:'center',gap:11},checkDot:{width:10,height:10,borderRadius:5},error:{color:'#e15a6b',padding:14},meetingHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',padding:16,borderBottomWidth:1,gap:10},grid:{padding:12,gap:12},primaryControls:{flexDirection:'row',padding:8,gap:4,borderTopWidth:1},primaryControl:{flex:1,alignItems:'center',gap:5,minWidth:0},controlIcon:{width:42,height:42,borderRadius:16,alignItems:'center',justifyContent:'center'},tile:{borderWidth:2,height:260,backgroundColor:'#101b2a',borderRadius:22,overflow:'hidden',alignItems:'center',justifyContent:'center',shadowColor:'#000',shadowOpacity:.2,shadowRadius:15,shadowOffset:{width:0,height:8},elevation:5},fullTile:{flex:1,height:undefined,borderRadius:0},tileCaption:{position:'absolute',bottom:8,left:8,right:8,backgroundColor:'#07101dcc',padding:10,borderRadius:13,flexDirection:'row',alignItems:'center',gap:8},tileName:{color:'white',flex:1},choiceGrid:{flexDirection:'row',flexWrap:'wrap',gap:10},choice:{minWidth:92,minHeight:48,paddingHorizontal:15,borderRadius:16,borderWidth:1,alignItems:'center',justifyContent:'center'},viewerToolbar:{zIndex:2,minHeight:58,paddingHorizontal:8,paddingVertical:6,backgroundColor:'#07101df2',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5,flexWrap:'wrap'},viewerAction:{minHeight:42,minWidth:42,paddingHorizontal:10,borderRadius:15,backgroundColor:'#ffffff18',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6},viewerActionText:{color:'white',fontSize:12,fontWeight:'700'},viewerParticipantsScroll:{flexGrow:0,backgroundColor:'#07101df2'},viewerParticipants:{paddingHorizontal:8,paddingBottom:8,gap:7},viewerParticipant:{minHeight:48,paddingHorizontal:9,paddingVertical:6,borderRadius:16,borderWidth:1,flexDirection:'row',alignItems:'center',gap:7},viewerParticipantName:{color:'white',fontSize:12,fontWeight:'700'},viewerParticipantSource:{color:'#ffffffa8',fontSize:10},viewerBody:{flex:1,minHeight:0},viewerStage:{flex:2,minWidth:0,minHeight:0,overflow:'hidden'},viewerChat:{flex:1,minWidth:280,minHeight:0,borderLeftWidth:1,borderColor:'#ffffff22'},viewerMessages:{padding:10,gap:9},composerWrap:{borderTopWidth:1,borderColor:'#ffffff22'},replyPreview:{minHeight:38,paddingHorizontal:10,flexDirection:'row',alignItems:'center',gap:8,borderLeftWidth:3},replyQuote:{padding:7,borderLeftWidth:2,borderRadius:7},messageHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},systemMessage:{alignSelf:'center',paddingVertical:7,paddingHorizontal:12,backgroundColor:'transparent'},emojiButton:{width:44,height:48,borderRadius:15,alignItems:'center',justifyContent:'center',backgroundColor:'#ffffff12'},emojiCategories:{paddingHorizontal:12,gap:8},emojiCategory:{minHeight:42,paddingHorizontal:12,borderWidth:1,borderRadius:14,alignItems:'center',justifyContent:'center'},emojiGrid:{padding:12,flexDirection:'row',flexWrap:'wrap',gap:8},emojiCell:{width:52,height:52,borderRadius:16,borderWidth:1,alignItems:'center',justifyContent:'center'},viewerComposer:{padding:8,flexDirection:'row',alignItems:'center',gap:7},viewerSend:{width:48,height:48,borderRadius:16,alignItems:'center',justifyContent:'center'},controls:{padding:10,paddingBottom:16,flexDirection:'row',flexWrap:'wrap',justifyContent:'center',gap:8,borderTopWidth:1},permissionRow:{flexDirection:'row',flexWrap:'wrap',gap:7},permissionChip:{minHeight:38,paddingHorizontal:11,borderRadius:13,borderWidth:1,alignItems:'center',justifyContent:'center'},adminNotice:{minHeight:46,padding:12,borderRadius:15,borderWidth:1,flexDirection:'row',alignItems:'center',gap:9},moderationEvent:{minHeight:42,paddingVertical:9,borderBottomWidth:1,flexDirection:'row',alignItems:'center',gap:8},bubble:{padding:16,borderRadius:18,gap:12},composer:{padding:10,flexDirection:'row',alignItems:'center',gap:8}
});
