...
test('window and clipboard IPC reject other frames and origins',()=>{
 ...
 const event={sender:wc,senderFrame:wc.mainFrame};handlers['meet:copy-invite'](event,'abc-def');assert.match(copied,/room=abc-def$/);
 assert.throws(()=>handlers['meet:copy-invite']({...event,senderFrame:{}},'abc-def'),/Forbidden/);
